...
int main(int argc, char ** argv)
{
  ...
  // My contribution: uncomment once accelerator code works
  ...
  CConv2DProxy convolver(true); // Deactivate logging. When true -> terminal verbose
  ...
  for (uint32_t iTest = 0; iTest < numSizes; ++ iTest) {
    ...
    */
    ...
    for (uint32_t repe = 0; repe < NUM_REPES; ++ repe) {
      ...
      // My contribution: call the accelerator (HW)
      convolver.Conv2D_HW(inputHW, outputHW, coeffsHW, channels, filters, width, height, 3, 3);

      // My contribution: debug printing for the accelerator
      if (channels == 3 && filters == 32) {
        printf("[HW DEBUG] numChannels=%u, numFilters=%u\n", channels, filters);
        printf("[HW DEBUG] coeffs[0]=%d, coeffs[1]=%d, coeffs[27]=%d\n", 
               coeffsHW[0], coeffsHW[1], coeffsHW[3*9]); 
    }
      ...
      // My contribution: call CompareVectors
      if (!CompareVectors(output, outputHW, currentOutputSize)) {
        printf("=========== Error: output from accelerator (HW) does NOT match SW output ===========\n");
      } else {
        printf("=========== OK! ===========\n");
      }
    }
