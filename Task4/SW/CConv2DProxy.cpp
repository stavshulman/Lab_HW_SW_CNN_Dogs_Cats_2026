      ...
      uint32_t convWidth, uint32_t convHeight)
{
  ...
  // My contribution:
  status = regs->control;
  status |= 0x01; // start bit: LSB of control register (see Vitis HLS document)
  regs->control = status;
  ...
  do {
    status = regs->control;
  } while ((status & 0x02) != 0x02); // ap_done bit: 2nd LSB of control register (see Vitis HLS document)
